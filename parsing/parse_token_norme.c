...
{
    if ((*tokens)->type == INPUT || (*tokens)->type == OUTPUT || (*tokens)->type == APPEND)
    {
        if (!ctx->current_command)
        {
            ctx->current_command = new_command();
        ...
        add_redirection(ctx->current_command, (*tokens)->type, (*tokens)->next->value);
 ...
 void parse_token_five(t_parse_context *ctx, t_token **tokens)
{
    if ((*tokens)->type == PIPE)
    {
        ...
        if ((*tokens)->next && (*tokens)->next->type != INPUT && (*tokens)->next->type != OUTPUT &&
